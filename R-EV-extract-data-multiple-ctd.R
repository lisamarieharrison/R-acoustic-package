...
for(i in 53:60){
  ...
  raw.files <- list.files(path = "F:/RAW/", pattern = raw.file.pattern, full.names = T) 
  ...
  dummy.def <- file(paste("F:/Region_definitions_files/region_def_file_stn_", i, ".evr", sep = ""), 'w')
  for (j in 1:9){
  ...
  EVImportRegionDef(EVFile, paste("F:/Region_definitions_files/region_def_file_stn_", i, ".evr", sep = ""), line_9)
  ...
  EVExportRegionSv(EVFile, '38H hri ex noise', paste("CTD_", i, sep = ""), paste("F:/Extracted data/38kHz/stn_", i, "_extracted_38khz.csv", sep =""))
